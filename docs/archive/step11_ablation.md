...
2. **Ablation 证明了组件的必要性排序**：Abstain > Retry > Grader/Decompose
3. **信号 vs 模型的工程判断**：reranker 客观信号比 LLM 主观判定更稳定（v1_no_grader ≈ v1_full）
4. **诚实评估评测盲区**：DECOMPOSE 在 81 题中触发率低（知识库单文档覆盖好 + probe 设计缺陷），但 counterfactual 证明分支活着
