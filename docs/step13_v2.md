...
1. **Architecture follows evidence**：四个失败案例统一到一个"evidence memory + hop state + missing-evidence detection"架构，不是四个 patch——展示架构抽象能力
2. **从"知道什么时候不回答"到"能自主获取缺失证据"**：Final Rescue > 0 是项目里程碑，证明 Agent 能解决 Fixed RAG 解决不了的问题
